...
Begin VB.Form frmckd_sales 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.ComboBox Combo1 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmckd_salesnew.frx":F172
         Left            =   9720
         List            =   "frmckd_salesnew.frx":F17C
      ...
      Begin MSDataListLib.DataList DataList1 
         Bindings        =   "frmckd_salesnew.frx":F18E
         Height          =   2400
         Left            =   120
         TabIndex        =   38
         TabStop         =   0   'False
   ...
   Begin MSDataListLib.DataCombo dtprm 
      Bindings        =   "frmckd_salesnew.frx":F1A5
      Height          =   315
      Left            =   12360
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   615
      Left            =   0
      Picture         =   "frmckd_salesnew.frx":F1BE
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7545
   End
...
If cmdsearch.Caption = "Clear Search" Then
   ...
   adostock.RecordSource = "SELECT *FROM stock_ckd"
   adostock.Refresh
...
ElseIf cmdsearch.Caption = "SEARCH" Then
  adostock.CommandType = adCmdText
  adostock.RecordSource = "SELECT *FROM stock_ckd WHERE (((stock_ckd.BID)  Like '" & Me.txtsearch.Text & "%'))"
  adostock.Refresh
    If adostock.Recordset.RecordCount = 0 Then
        adostock.CommandType = adCmdText
        adostock.RecordSource = "SELECT *FROM stock_ckd WHERE (((stock_ckd.BIKE_DETAILS)  Like '" & Me.txtsearch.Text & "%'))"
        adostock.Refresh
    End If
    If adostock.Recordset.RecordCount = 0 Then
...
On Error GoTo errorhandler
 ...
 'setting up a new transaction ID without adding new
    ...
    Else: adoinvoice.Recordset.Sort = "CKDTID ASC"
    adoinvoice.Recordset.MoveLast
        varTID = Mid(adoinvoice.Recordset.Fields(0).Value, 5, 11)
        txtinv_date.Text = Format(Date, "dd/MM/yyyy")
 ...
 'initialsing objects
    ...
    With adoinvoice_details
        ...
        .RecordSource = "SELECT *FROM ckd_details"
    ...
    Else:
        adoinvoice.Recordset.Sort = "CKDTID ASC"
        adoinvoice.Recordset.MoveLast
        varTID = Mid(adoinvoice.Recordset.Fields(0).Value, 5, 11)
        txtinv_date.Text = Format(Date, "dd/MM/yyyy")
        txtinv_no.Text = "CKDT" + CStr(varTID + 1)
    ...
    Else:
        ...
        b = Val(adostock.Recordset.Fields(2).Value)
            If Val(adostock.Recordset.Fields(2).Value) = 0 Then
                ...
                txtqty.Text = 0
            ElseIf a > b Then
                MsgBox "Sorry you only have" & " " & adostock.Recordset.Fields(2).Value & " " & "Bikes left", vbOKOnly + vbInformation, "BORAC SALES SYSTEM"
                txtqty.Text = ""
            End If
    End If
errorhandler:
